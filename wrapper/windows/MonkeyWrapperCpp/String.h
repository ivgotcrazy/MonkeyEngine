...
{
	return sizeof(StringData) + sizeof(T) * (m_capacity + 1);
}
...
{
	StringData* r = reinterpret_cast<StringData*>(iliveMalloc(getSize()));
	r->m_length = m_length;
...
template<typename T> inline TStringBase<T>
		...
		TStringBase<T>::vformat(const T* format, va_list argptr)
	{
		...
		{
			_getData().unRef();
			m_string = StringData::Create(MIN_STRING_CAPACITY)->getData();
		}
		while (true)
		{
